...
function PRCard({ pr }: { pr: PullRequest }) {
  ...
  // Build preview URL if we extracted source_repo from the PR diff
  const previewUrl = pr._source_repo
    ? `/packs`
    : null;
  ...
  return (
    <a
      ...
      className="flex items-center gap-3 rounded-lg border border-surface-border bg-surface-card p-4 transition-all duration-200 hover:border-gold/50 group"
    >
      {/* PR icon */}
      ...
      <div className="flex-1 min-w-0">
        <div className="text-sm font-medium text-text-primary group-hover:text-gold transition-colors truncate">
          {pr.title}
        </div>
